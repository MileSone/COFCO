        ...
        function (oj, ko, $)
        {
            function CustomerViewModel() {
                ...
                self.init = function (input) {
                    ...
                    //
                    // 第三个柱形图开始
                    self.stackValue_income_area_column = ko.observable('');
                    self.orientationValue_income_area_column = ko.observable('');

                    var income_area_columnSeries = [];
                    var income_area_columnGroups = [];

                    self.comboSeriesValue_income_area_column = ko.observableArray(income_area_columnSeries);
                    self.comboGroupsValue_income_area_column = ko.observableArray(income_area_columnGroups);
                    self.yMax_income_area_column = 10000;

                    var converterFactory_income_area_column = oj.Validation.converterFactory('number');
                    var converterOptions_income_area_column = {style: 'percent'};
                    self.y2Converter_income_area_column = converterFactory_income_area_column.createConverter(converterOptions_income_area_column);
                    ...
                    // 第四个柱形图结束
                    // 第五个柱形图开始
                    self.stackValue_profit_area_column = ko.observable('');
                    self.orientationValue_profit_area_column = ko.observable('');

                    var profit_area_columnSeries = [];
                    var profit_area_columnGroups = [];

                    self.comboSeriesValue_profit_area_column = ko.observableArray(profit_area_columnSeries);
                    self.comboGroupsValue_profit_area_column = ko.observableArray(profit_area_columnGroups);
                    self.yMax_profit_area_column = 10000;

                    var converterFactory_profit_area_column = oj.Validation.converterFactory('number');
                    var converterOptions_profit_area_column = {style: 'percent'};
                    self.y2Converter_profit_area_column = converterFactory_profit_area_column.createConverter(converterOptions_profit_area_column);
                    ...
                    // 第五个柱形图结束
                    // 第六个柱形图开始
                    self.stackValue_stock_area_column = ko.observable('');
                    self.orientationValue_stock_area_column = ko.observable('');

                    var stock_area_columnSeries = [];
                    var stock_area_columnGroups = [];

                    self.comboSeriesValue_stock_area_column = ko.observableArray(stock_area_columnSeries);
                    self.comboGroupsValue_stock_area_column = ko.observableArray(stock_area_columnGroups);
                    self.yMax_stock_area_column = 10000;

                    var converterFactory_stock_area_column = oj.Validation.converterFactory('number');
                    var converterOptions_stock_area_column = {style: 'percent'};
                    self.y2Converter_stock_area_column = converterFactory_stock_area_column.createConverter(converterOptions_stock_area_column);
                    ...
                    // 第六个柱形图结束
                    // 第七个柱形图开始
                    self.stackValue_stock_alert_area_column = ko.observable('');
                    self.orientationValue_stock_alert_area_column = ko.observable('');

                    var stock_alert_area_columnSeries = [];
                    var stock_alert_area_columnGroups = [];

                    self.comboSeriesValue_stock_alert_area_column = ko.observableArray(stock_alert_area_columnSeries);
                    self.comboGroupsValue_stock_alert_area_column = ko.observableArray(stock_alert_area_columnGroups);
                    self.yMax_stock_alert_area_column = 10000;

                    var converterFactory_stock_alert_area_column = oj.Validation.converterFactory('number');
                    var converterOptions_stock_alert_area_column = {style: 'percent'};
                    self.y2Converter_stock_alert_area_column = converterFactory_stock_alert_area_column.createConverter(converterOptions_stock_alert_area_column);
...
//============================================================================饼图开始======================================================
                ...
                self.reInitView = function(){
                    ...
                    $.ajax({
                        type: "GET",
                        url: "http://"+baseUrl+"/Area/listAreaData" + stringFilter,
                        dataType: "json",
                        success: function (resp) {
                            ...
                            //
                            //
                            self.comboSeriesValue_income_area_column( resp.grossProfit.chart1.data.series);
                            self.comboGroupsValue_income_area_column(resp.grossProfit.chart1.data.groups);
                            //self.label_income_area_column(resp.grossProfit.chart1.chartname);
                            ...
                            self.comboSeriesValue_income_area_channel_customer_column( resp.grossProfit.chart3.data.series);
                            self.comboGroupsValue_income_area_channel_customer_column(resp.grossProfit.chart3.data.groups);
                            //self.label_income_area_channel_customer_column(resp.grossProfit.chart3.chartname);

                            self.comboSeriesValue_income_area_column( resp.grossProfit.chart4.data.series);
                            self.comboGroupsValue_income_area_column(resp.grossProfit.chart4.data.groups);
                            //self.label_income_area_column(resp.grossProfit.chart4.chartname);
                        },
                        error: function (e) {
                ...
                self.sale_area_column = function () {
                    ...
                    $.get(serverURL, function (data) {
                        var obj = eval('(' + data + ')');
                        sale_area_columnSeries = [];
                        sale_area_columnGroups = [];
                        for (var i = 0; i < obj.groups.length; i++) {
                            sale_area_columnGroups.push(obj.groups[i]);
                        }
                        for (var i = 0; i < obj.series.length; i++) {
                            sale_area_columnSeries.push(obj.series[i]);
                        }
                        self.comboSeriesValue_sale_area_column(sale_area_columnSeries);
                        self.comboGroupsValue_sale_area_column(sale_area_columnGroups);
                    });
                };
                ...
                self.income_area_column = function () {
                    ...
                    $.get(serverURL, function (data) {
                        ...
                        income_area_columnGroups = [];
                        for (var i = 0; i < obj.groups.length; i++) {
                            income_area_columnGroups.push(obj.groups[i]);
                        }
                        for (var i = 0; i < obj.series.length; i++) {
                            income_area_columnSeries.push(obj.series[i]);
                        }
                        self.comboSeriesValue_income_area_column(income_area_columnSeries);
                        self.comboGroupsValue_income_area_column(income_area_columnGroups);
                    });
                };
                ...
                self.stock_area_column = function () {
                    ...
                    $.get(serverURL, function (data) {
                        ...
                        stock_area_columnGroups = [];
                        for (var i = 0; i < obj.groups.length; i++) {
                            stock_area_columnGroups.push(obj.groups[i]);
                        }
                        for (var i = 0; i < obj.series.length; i++) {
                            stock_area_columnSeries.push(obj.series[i]);
                        }
                        self.comboSeriesValue_stock_area_column(stock_area_columnSeries);
                        self.comboGroupsValue_stock_area_column(stock_area_columnGroups);
                    });
                };
                ...
                self.stock_alert_area_column = function () {
                    ...
                    $.get(serverURL, function (data) {
                        ...
                        stock_alert_area_columnGroups = [];
                        for (var i = 0; i < obj.groups.length; i++) {
                            stock_alert_area_columnGroups.push(obj.groups[i]);
                        }
                        for (var i = 0; i < obj.series.length; i++) {
                            stock_alert_area_columnSeries.push(obj.series[i]);
                        }
                        self.comboSeriesValue_stock_alert_area_column(stock_alert_area_columnSeries);
                        self.comboGroupsValue_stock_alert_area_column(stock_alert_area_columnGroups);
                    });
                };
